...
class Custom_packages extends CI_Controller
{
    ...
    public function index() {
        ...
        $data['dripfeed_detail_followers'] = $this->common_model->get_table_data('tbl_dripfeed', '*', array('dripfeed_for' => 'followers'));
    ...
    public function confirm_followers_order() {
        ...
        $dripfeed = $this->input->post('followers_dripfeed');
        if ($dripfeed == "yes") {
            $dripfeed = 'no';
        } else {
            $dripfeed = 'yes';
        ...
        if ($dripfeed == 'yes') {
            $total_price = $this->input->post('followers_price_value') + $dripfeed_price;
        } else {
        ...
        $insert_data = array(
            ...
            'dripfeed_minutes' => $dripfeed_minutes,
    ...
    public function confirm_likes_order() {
        ...
        $dripfeed = $this->input->post('likes_dripfeed');
        if ($dripfeed == "yes") {
            $dripfeed = 'no';
        } else {
            $dripfeed = 'yes';
        ...
        if ($dripfeed == 'yes') {
            $total_price = $this->input->post('likes_price_value') + $dripfeed_price;
        } else {
    ...
    public function confirm_views_order() {
        is_user_in();
        $custom_order_code = time() . uniqid();
        $dripfeed = $this->input->post('views_dripfeed');
        if ($dripfeed == "yes") {
            $dripfeed = 'no';
        } else {
            $dripfeed = 'yes';
        ...
        if ($dripfeed == 'yes') {
            $total_price = $this->input->post('views_price_value') + $dripfeed_price;
        } else {
    ...
    public function confirm_likesViews_order() {
        ...
        $dripfeed = $this->input->post('likes_views_dripfeed');
        if ($dripfeed == "yes") {
            $dripfeed = 'no';
        } else {
            $dripfeed = 'yes';
        ...
        if (isset($likes_send)) {
            if ($dripfeed == 'yes') {
                $total_price_likes = $this->input->post('likes_price_value') + $dripfeed_price;
            } else {
...
';
            ...
            $email_qry = $this->smtp_email->send('no-reply@socialmediagainer.com', 'Social Media Gainer', $this->session->userdata('email'), 'Instagram Order Purchase', $email_msg);
    ...
    function validate_dripfeed() {
        is_user_in();
        $dripfeed_id = $this->input->post('dripfeed_id');
        $min_qty = $this->input->post('min_qty');
        $post_qty = $this->input->post('post_qty');

        if (!empty($dripfeed_id)) {
            $dripfeed_data = $this->common_model->get_table_data('tbl_dripfeed', '*', array('dripfeed_id' => $dripfeed_id));
            $dripfeed_run = $dripfeed_data[0]['dripfeed_run'];

            $calculate_post_value = $post_qty / $dripfeed_run;
            $min_req_quantity = $min_qty * $dripfeed_run;

            if ($calculate_post_value < $min_qty) {
                $response = 'error';
            } else {
                $response = 'correct';
            }
            ...
            $data = array(
                'response' => $response,
                'min_qty' => $min_req_quantity,
                'dripfeed_run' => $dripfeed_run,
            );
        } else {
            $data = array(
                'response' => 'correct',
    ...
    function validate_dripfeed_likes_views() {
        is_user_in();
        $dripfeed_id = $this->input->post('dripfeed_id');
        $min_qty_views = $this->input->post('min_qty_views');
        $min_qty_likes = $this->input->post('min_qty_likes');
        $post_qty_views = $this->input->post('post_qty_views');
        $post_qty_likes = $this->input->post('post_qty_likes');
        ...
        if (!empty($dripfeed_id)) {
            ...
            if ($calculate_post_views < $min_qty_views) {
                $response_views_2 = 'error';
            } else {
                $response_views_2 = 'correct';
            }

            if ($calculate_post_likes < $min_qty_likes) {
                $response_likes_2 = 'error';
            } else {
                $response_likes_2 = 'correct';
            }

            $data = array(
                'response_likes_2' => $response_likes_2,
                'response_views_2' => $response_views_2,
                'min_qty_likes_2' => $min_req_quantity_likes,
                'min_qty_views_2' => $min_req_quantity_views,
                'dripfeed_run' => $dripfeed_run,
            );
        } else {
            $data = array(
