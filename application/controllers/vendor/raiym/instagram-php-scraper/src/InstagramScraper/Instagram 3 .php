...
class Instagram
{
    ...
    public function getMedias($username, $count = 20, $maxId = '')
    {
    	...
    	while ($index < $count && $isMoreAvailable) {
    		...
    		$nodes = array_reverse($arr['data']['user']['edge_owner_to_timeline_media']['edges']);


            //echo '<pre>'; print_r($nodes); echo '</pre>'; exit;

            uasort($nodes, function ($i, $j) {
                $a = $i['node']['taken_at_timestamp'];
                $b = $j['node']['taken_at_timestamp'];
                if ($a == $b) return 0;
                elseif ($a > $b) return 1;
                else return -1;
            });
        ...
        while ($index < $count && $isMoreAvailable) {
            ...
            uasort($nodes, function ($i, $j) {
                $a = $i['taken_at_timestamp'];
                $b = $j['taken_at_timestamp'];
                if ($a == $b) return 0;
                elseif ($a > $b) return 1;
                else return -1;
            });
